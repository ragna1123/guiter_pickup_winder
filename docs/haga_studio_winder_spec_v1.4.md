...
## 更新概要（v1.3 → v1.4）
- RPM計測窓を**可変式（FAST/SLOW）**に変更、EMA平滑化を追加  
- RUN↔PAUSEの割り込み切替を**安全側（明示的に無効化→再登録）**へ修正  
- エラー復帰時に**再開／リセット再開／メニュー戻り**を選択可能化  
- LEDは**初期選択方向を保持**し、逆転時には切り替えない（画面アイコンで方向表示）  
- センサノイズ対策（最短パルス幅フィルタ）を追加  
- RUN/COUNTモード処理を共通関数`run_loop()`に整理  
...
|------|------|
| 再開 | 巻き数・設定保持 |
| リセット再開 | 巻き数・RPMリセット、設定保持 |
| メニュー | リザルト画面 → メニューへ戻る |

UI例：右端に`PUSH:決定 / 回して選択`
...
| 状態 | 青LED | 赤LED | 備考 |
|------|--------|--------|------|
| RUN中 | 選択方向LEDを点灯 | もう一方消灯 | 手巻き方向が逆でも変更しない |
| ポーズ中 | 選択方向LEDを点滅 | もう一方消灯 | |
| 待機中 | 選択方向LEDを点滅 | もう一方消灯 | |
